            ...
            Phase 05  Enhancement eval                  │
                    ...
                    └─────────────────┬─────────────────┘
                              ...
                              Phase 07  Dropout ablation
                                      ▼
                              Phase 08  Inference & end-to-end pipeline
                                      ▼
                              Phase 10  Report & submission
```

> **Rescoped 2026-08-14 (DEV-004).** Phase 08 was "Bonus: chained scanner"; it is now the
> **mandatory** inference phase, because the two pipelines it delivers are `[REQ-29]`,
> `[REQ-32]` and `[REQ-46]`, and `[REQ-49]` has the teaching staff run them on unseen
> photos. The chained scanner is glue over those two and is delivered in the same phase.
> **Phase 09 (differentiable joint fine-tuning) is dropped** — ADR-012 always made it
> conditional, its entry conditions are not met, and skipping it is the designed behaviour
> rather than a deviation. `phase-09-bonus-joint.md` is retained for the reasoning trail.
...
| [08](phase-08-inference-endtoend.md) | **Inference & end-to-end pipeline** | Three entry points survive six robustness cases without crashing; chain evaluated with annotated *and* predicted corners | 05, 06 |
| ~~09~~ | ~~Bonus: joint fine-tune~~ | **DROPPED** — DEV-004 | — |
