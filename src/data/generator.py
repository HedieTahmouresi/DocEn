...
def render_heatmaps(
    ...
    for i in range(4):
        ...
        gauss = np.exp(-dist_sq / (2.0 * sigma ** 2), dtype=np.float32)
...
class SyntheticSampleGenerator:
    ...
    def __init__(
        ...
        self.scale_factor_range = res_cfg.get("scale_factor", (1.5, 4.5))

        photo_cfg = gen_cfg.get("photometric", {})
        self.contrast_range = photo_cfg.get("contrast", (0.4, 1.6))
        self.brightness_range = photo_cfg.get("brightness", (-50.0, 50.0))
        self.channel_gain_range = photo_cfg.get("channel_gain", (0.75, 1.25))

        illum_cfg = gen_cfg.get("illumination", {})
        self.gradient_range = illum_cfg.get("gradient_range", (0.55, 1.15))
        self.shadow_count_range = illum_cfg.get("shadow_count", (0, 3))
        self.shadow_opacity_range = illum_cfg.get("shadow_opacity", (0.15, 0.55))
        self.shadow_blur_range = illum_cfg.get("shadow_blur", (15, 91))
        self.shadow_probability = illum_cfg.get("shadow_probability", 0.7)

        sensor_cfg = gen_cfg.get("sensor", {})
        self.blur_kernel_range = sensor_cfg.get("blur_kernel", (3, 9))
        self.blur_sigma_range = sensor_cfg.get("blur_sigma", (0.5, 3.0))
        self.motion_blur_prob = sensor_cfg.get("motion_blur_prob", 0.25)
        self.noise_sigma_range = sensor_cfg.get("noise_sigma", (3.0, 30.0))
        self.darkness_scaled_noise = sensor_cfg.get("darkness_scaled_noise", True)

        comp_cfg = gen_cfg.get("compression", {})
        self.jpeg_quality_range = comp_cfg.get("jpeg_quality", (20, 90))
    ...
    def generate(
        ...
        if not photometrics_off:
            # -----------------------------------------------------------------
            # Step 2: Resolution Loss (Downscale x[1.5, 4.5] -> Upscale)
            # -----------------------------------------------------------------
            ...
            # -----------------------------------------------------------------
            # Step 3: Brightness, Contrast, Colour Cast
            # -----------------------------------------------------------------
            ...
            # -----------------------------------------------------------------
            # Step 4: Illumination Gradient x Soft Shadows
            # -----------------------------------------------------------------
            ...
            # -----------------------------------------------------------------
            # Step 5: Gaussian Blur -> Gaussian Noise (in that order!)
            # -----------------------------------------------------------------
